...
export function JournalEntryPhotos({ photos, entryTitle }: { photos: { id: string; url: string }[]; entryTitle: string }) {
  ...
  return (
    <>
      <div style={{ display: "flex", gap: "0.3125rem", marginBottom: "0.5625rem" }}>
        {photos.map(({ id, url }, i) => (
          <button key={id} type="button" className="kin-photo-thumb" onClick={() => setOpenIndex(i)} aria-label={`Open photo ${i + 1} from ${entryTitle}`}>
      ...
      {openIndex !== null && (
        <PhotoViewer
          items={photos.map(({ id, url }, i) => ({ url, alt: `Photo ${i + 1} from ${entryTitle}`, photo: { kind: "journal", id } }))}
